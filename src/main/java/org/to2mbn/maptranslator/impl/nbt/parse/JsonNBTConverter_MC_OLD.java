...
class JsonNBTConverter_MC_OLD implements JsonNBTConverter {

	private static final Pattern INT_ARRAY_MATCHER = Pattern.compile("\\[[-+\\d|,\\s]+\\]");

	static final JsonNBTConverter INSTANCE = new JsonNBTConverter_MC_OLD();

	@Override
	public NBTCompound parse(String json) throws NBTException {
		json = json.trim();
		if (!json.startsWith("{")) {
	...
	private int countTopTags(String str) throws NBTException {
		int count = 0;
	...
	private JsonNBTConverter_MC_OLD.Any nameValueToNBT(String key, String value) throws NBTException {
		value = value.trim();

		if (value.startsWith("{")) {
			value = value.substring(1, value.length() - 1);
			JsonNBTConverter_MC_OLD.Compound compound;
			String s1;

			for (compound = new JsonNBTConverter_MC_OLD.Compound(key); value.length() > 0; value = value.substring(s1.length() + 1)) {
			...
			value = value.substring(1, value.length() - 1);
			JsonNBTConverter_MC_OLD.List list;
			String s;

			for (list = new JsonNBTConverter_MC_OLD.List(key); value.length() > 0; value = value.substring(s.length() + 1)) {
			...
			return new JsonNBTConverter_MC_OLD.Primitive(key, value);
		}
	}

	private JsonNBTConverter_MC_OLD.Any getTagFromNameValue(String str, boolean isArray) throws NBTException {
		String key = locateKey(str, isArray);
		String value = locateValue(str, isArray);
		return nameValueToNBT(key, value);
	}

	private String nextNameValuePair(String str, boolean isCompound) throws NBTException {
	...
	private String locateValueAt(String str, int index) throws NBTException {
		Stack<Character> stack = new Stack<>();
		int i = index + 1;
		boolean escaped = false;
		boolean flag1 = false;
	...
	private String locateKey(String str, boolean isArray) throws NBTException {
		if (isArray) {
			str = str.trim();
	...
	private String locateValue(String str, boolean isArray) throws NBTException {
		if (isArray) {
			str = str.trim();
	...
	private int getNextCharIndex(String str, char targetChar) {
		int i = 0;
	...
	private boolean isCharEscaped(String str, int index) {
	...
	static class Compound extends JsonNBTConverter_MC_OLD.Any {

		protected java.util.List<JsonNBTConverter_MC_OLD.Any> tagList = new ArrayList<>();
		...
		public NBT parse() throws NBTException {
			...
			for (JsonNBTConverter_MC_OLD.Any jsontonbt$any : this.tagList) {
	...
	static class List extends JsonNBTConverter_MC_OLD.Any {
		...
		public NBT parse() throws NBTException {
			...
			int i = 0;
			for (JsonNBTConverter_MC_OLD.Any child : this.tagList) {
	...
	static class Primitive extends JsonNBTConverter_MC_OLD.Any {
